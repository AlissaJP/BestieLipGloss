    ...
    <div className="min-h-screen bg-[#F2E9E1] p-6">
      <div className="max-w-3xl mx-auto">
        <Link href="/admin/dashboard" className="inline-flex items-center gap-2 font-lato text-sm text-gray-500 hover:text-primary transition-colors mb-6">
          <ArrowLeft size={15} />{tz.backDashboard}
        </Link>

        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="font-playfair font-bold text-2xl text-gray-800">{tz.title}</h1>
            <p className="font-lato text-sm text-gray-500 mt-0.5">{zones.length} zone{zones.length !== 1 ? 's' : ''}</p>
          </div>
          <button
            onClick={openCreate}
          ...
          >
            <Plus size={15} />{tz.addBtn}
          </button>
        </div>

        {loading ? (
          <div className="text-center py-16 font-lato text-gray-400">{tz.loading}</div>
        ) : (
              ...
              <div key={zone.id} className={`bg-white rounded-2xl p-4 border-2 flex items-center gap-4 ${zone.actif ? 'border-pink-100' : 'border-gray-100 opacity-60'}`}>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button
                    ...
                    title={zone.actif ? tz.disableTitle : tz.enableTitle}
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
              <h2 className="font-playfair font-bold text-lg text-gray-800 mb-5">
                {editing ? tz.formEdit : tz.formNew}
              </h2>
              <div className="space-y-4">
                <div>
                  <label className="font-lato text-sm font-medium text-gray-700 block mb-1.5">{tz.fieldName}</label>
                  <input value={form.nom_zone} onChange={(e) => setForm((f) => ({ ...f, nom_zone: e.target.value }))} className={inputCls} placeholder={tz.fieldNamePlaceholder} />
                </div>
                <div>
                  <label className="font-lato text-sm font-medium text-gray-700 block mb-1.5">{tz.fieldFee}</label>
                ...
                <div>
                  <label className="font-lato text-sm font-medium text-gray-700 block mb-1.5">{tz.fieldThreshold} <span className="text-gray-400 font-normal">{tz.fieldThresholdOpt}</span></label>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-lato text-sm text-gray-700">{tz.fieldActive}</span>
                </label>
              </div>
              <div className="flex gap-3 mt-6">
                <button onClick={() => setShowForm(false)} className="flex-1 border border-gray-200 text-gray-600 font-lato text-sm py-2.5 rounded-xl hover:border-gray-300 transition-colors">{tz.cancel}</button>
                <button onClick={handleSave} disabled={!form.nom_zone || !form.frais_htg} className="flex-1 bg-primary hover:bg-pink-400 disabled:opacity-50 text-white font-lato text-sm font-semibold py-2.5 rounded-xl transition-colors">{tz.save}</button>
              </div>
            </div>
          </div>
        )}
